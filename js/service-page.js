...
(function () {
  function initGalleryLightbox() {
    ...
    /* Open on fullscreen button click — use currently active slide */
    btn.addEventListener('click', function (e) {
      e.stopPropagation();
      /* Find active index from the scroll track transform */
      var track = document.getElementById('galleryScrollTrack');
      var slideH = display.offsetHeight || 320;
      var tx = track ? parseFloat((track.style.transform || '').replace(/[^0-9\-\.]/g, '')) || 0 : 0;
      var idx = Math.round(Math.abs(tx) / slideH);
      openLightbox(idx);
    });

    /* Also open on double-click of main display image */
